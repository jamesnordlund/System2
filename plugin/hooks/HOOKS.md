...
# Test syntax
for f in hooks/*.py; do python3 -m py_compile "$f"; done
```
...
hooks:
  PostToolUse:
    - matcher: "Edit|Write"
      hooks:
        - type: command
          command: 'python3 "${CLAUDE_PLUGIN_ROOT}/hooks/auto-formatter.py"'  # Format first
        - type: command
          command: 'python3 "${CLAUDE_PLUGIN_ROOT}/hooks/type-checker.py"'    # Then type check
```
...
```bash
python3 -m py_compile hooks/dangerous-command-blocker.py
```
...
TOOL_NAME=Bash TOOL_INPUT='{"command":"rm -rf /"}' \
  python3 hooks/dangerous-command-blocker.py
echo "Exit code: $?"

# Test sensitive file protector
TOOL_NAME=Read TOOL_INPUT='{"file_path":".env"}' \
  python3 hooks/sensitive-file-protector.py
echo "Exit code: $?"

# Test TTS
python3 hooks/tts-notify.py stop
```
...
| Permission denied | Script not executable | Run `chmod +x hooks/*.py` |
